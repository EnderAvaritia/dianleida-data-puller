...
地理编码结果自动缓存到 `output/geocode_cache.json`（每 20 条写一次 + 结束时兜底）。
第二次运行相同地址直接命中缓存，不会重复请求 API。

**两阶段策略**：先串行扫描缓存（零开销），只将未命中的地址提交到线程池并行请求。
**Ctrl+C 安全**：中断后立即停止新请求，已完成的缓存不会丢，下次继续。
